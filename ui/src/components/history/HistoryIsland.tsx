...
function HistoryIslandInner() {
  ...
  if (loading) {
    return (
      <div className={styles.postList} aria-busy="true" aria-label="Loading history">
        {Array.from({ length: 3 }).map((_, i) => (
          <Card key={i} padding="none">
            <div className={styles.cardInner}>
              <Skeleton width={96} height={96} radius="var(--radius)" />
              <div className={styles.cardBody}>
                <Skeleton width={90} height={11} />
                <Skeleton width="60%" height={16} />
                <Skeleton width={140} height={11} />
                <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                  <Skeleton width={56} height={28} radius="var(--radius-sm)" />
  ...
  return (
    <ToastProvider>
      <div className={styles.page}>
        <PageHeader title="History" subtitle="All posts generated by newspapper." />
        <HistoryIslandInner />
      </div>
    </ToastProvider>
  );
