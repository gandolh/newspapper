...
function drawGridTexture(ctx: SKRSContext2D, gridSize = 32, opacity = 0.05) {
  ctx.strokeStyle = `rgba(27, 28, 28, ${opacity})`;
  ctx.lineWidth = 1;
...
function renderTitle(
  ...
  if (type === "title-main") {
    renderTitleMain(ctx, slide, d);
  } else if (type === "title-question") {
    renderTitleQuestion(ctx, slide, d);
  } else if (type === "title-statement") {
    renderTitleStatement(ctx, slide, d);
  } else {
    renderTitleMain(ctx, slide, d);
  }
  ctx.fillText(
